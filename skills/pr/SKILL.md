---
name: pr
description: 建立 Pull Request。限制僅能在 hotfix/* 或 develop 分支執行，且 PR 內容須為繁體中文。當使用者輸入 /git-flow-agy:pr 時觸發。
---
# Pull Request
...
2. **分支檢查**：
   - 若目前分支不是 `hotfix/*` 且不是 `develop`，必須拒絕執行，並向使用者顯示：「目前分支為 <分支名稱>。PR 僅允許由 hotfix 或 develop 分支發起。」
3. **PR 內容產生**：
...
4. **環境檢查與建立 PR**：
   ...
   - **若已安裝且已登入 (情境 A)**：
     - 根據目前分支，決定目標分支（通常 hotfix 對應 main/master；develop 對應 main/master）。
