...
description: 建立 Pull Request。限制僅能在 hotfix/* 或 release/* 分支執行，且 PR 內容須為繁體中文。當使用者輸入 /antigravity-git-flow:agy-git-flow:github-pr 時觸發。
---
...
1. 當使用者輸入 `/antigravity-git-flow:agy-git-flow:github-pr`。
...
6. 第六步：環境檢查與建立 PR
   ...
   - 若未安裝或未登入（情境 B）：
     停止後續所有動作，並提示使用者：「請先安裝 GitHub CLI（gh）並執行 `gh auth login` 完成登入後，再重新輸入 `/antigravity-git-flow:agy-git-flow:github-pr` 繼續執行。」
