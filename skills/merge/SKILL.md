---
name: antigravity-git-flow:agy-git-flow:merge
description: 依照 Git Flow 規則執行分支合併。當使用者輸入 /antigravity-git-flow:agy-git-flow:merge 時觸發。
---
...
1. 當使用者輸入 `/antigravity-git-flow:agy-git-flow:merge`。
2. 當使用者要求「合併分支」時。
...
4. 第四步：依分支類型執行合併邏輯（切換至目標分支後先執行 `git pull --ff-only` 同步，且合併指令一律加上 `--no-ff` 保留節點）：
   ...
   - 若在 release/* 或 hotfix/* 分支：
     ...
     ```
     - 若選擇「是（建立 PR）」：
       1. 從技能列表中讀取並執行 `antigravity-git-flow:agy-git-flow:github-pr`（或 `github-pr`）技能。
