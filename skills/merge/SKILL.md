...
3. 根據目前的分支類型執行對應的合併邏輯（注意：所有合併動作都必須加上 `--no-ff` 以保留節點）：
   - **若在 `feature/*`, `release/*` 或 `hotfix/*` 分支**：
     *(註：因為 release/hotfix 合併至 main 的操作已透過發佈 PR 於網頁端完成，因此本地只需將其合併回 develop 同步即可)*
     1. 檢查 `develop` 分支是否存在。若不存在，請從目前的主分支建立 `develop` 分支（`git checkout -b develop`）。
     2. 切換至 `develop` 分支（`git checkout develop`）。
     3. 執行合併（`git merge --no-ff <current-branch>`）。
