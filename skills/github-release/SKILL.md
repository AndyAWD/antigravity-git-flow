...
description: 建立 GitHub Release。自動擷取 commit 紀錄產生中英文雙語 Changelog。當使用者輸入 /antigravity-git-flow:agy-git-flow:github-release 時觸發。
---
...
1. 當使用者輸入 `/antigravity-git-flow:agy-git-flow:github-release`。
2. 當使用者提及「幫我建立 release」、「發布版本」時。

## 執行的實作步驟

1. 第一步：確認版本號
   呼叫 `ask_question` 工具詢問使用者要發布的版本號（例如 `v1.8.0`）。
...
4. 第四步：環境檢查與建立 Release
   ...
   - 若未安裝或未登入：
     停止後續所有動作，並提示使用者：「請先安裝 GitHub CLI（gh）並執行 `gh auth login` 完成登入後，再重新執行指令。」
